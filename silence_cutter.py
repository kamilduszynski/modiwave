...
def main() -> int:
    ...
    silence_audio_duration = math.modf(
        np.round(silence_sample_points / sampling_freq / 60, 2)
    )
    noise_audio_duration = math.modf(
        np.round(noise_sample_points / sampling_freq / 60, 2)
    )
    ...
    print(
        f"|original audio duration: {int(original_audio_duration[1])}min {int(60*original_audio_duration[0])}s"
    )
    print(f"|")
    print(f"|silence samples:         {silence_time_array.shape[0]}")
    print(
        f"|silence audio duration:  {int(silence_audio_duration[1])}min {int(60*silence_audio_duration[0])}s"
    )
    print(f"|")
    print(f"|noise samples:           {noise_time_array.shape[0]}")
    print(
        f"|noise audio duration:    {int(noise_audio_duration[1])}min {int(60*noise_audio_duration[0])}s"
    )
    print(f"|")
    ...
    wavfile.write("audio/audio_silence.wav", sampling_freq, sound[silence_parts])
    wavfile.write("audio/audio_no_silence.wav", sampling_freq, sound[noise_parts])

    sound = AudioSegment.from_wav("audio/audio.wav")
    sound = sound.set_channels(1)
    sound.export("audio/audio_mono.wav", format="wav")
